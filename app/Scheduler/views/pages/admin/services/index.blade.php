...
@section('content')
		...
		<div class="btn-toolbar">
			...
			<div class="btn-group">
				...
				<ul class="dropdown-menu" role="menu">
					<li><a href="{{ URL::route('admin.service.createOneToOne') }}">One occurrence, one participant</a></li>
					<li><a href="{{ URL::route('admin.service.createOneToMany') }}">One occurrence, many participants</a></li>
					<li><a href="{{ URL::route('admin.service.createManyToMany') }}">Many occurrences, many participants</a></li>
				</ul>
			</div>
		</div>
	</div>
	...
	<ul class="nav nav-tabs">
		@foreach ($categories as $category)
			<li><a href="#{{ strtolower(str_replace(' ', '', $category->name)) }}" data-toggle="tab">{{ $category->name }}</a></li>
		@endforeach
	</ul>

	<div class="tab-content">
	@foreach ($categories as $category)
		<div id="{{ strtolower(str_replace(' ', '', $category->name)) }}" class="tab-pane">
			@if (count($services[$category->id]) > 0)
			...
			@else
				<div class="alert alert-warning">No {{ Str::singular($category->name) }} services found.</div>
			@endif
		</div>
	@endforeach
	</div>
