...
@if ($_currentUser->isStaff())
	...
	<ul>
		...
		<li class="{{ (Request::is('admin/reports*') ? 'active' : '') }}"><a href="{{ URL::route('admin.reports.index') }}"><span class="icn-size-16">{{ $_icons['users'] }}</span>Report Center</a></li>
	</ul>
@endif
...
@if (Auth::check())
	<ul class="buttons">
		...
		@if ($_currentUser->isStaff())
			<li><a href="{{ URL::route('admin.staff.block') }}"><span class="icn-size-16">{{ $_icons['add'] }}</span>Block My Calendar</a></li>
		@endif
	</ul>
@endif