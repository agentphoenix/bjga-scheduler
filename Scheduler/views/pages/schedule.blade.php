...
@section('content')
	...
	@if (count($schedule) > 0)
		@foreach ($schedule as $days => $appointments)
			...
			@elseif ($days === 1)
				<h2>Tomorrow <small>{{ $now->addDay()->format(Config::get('bjga.dates.dateNoDay')) }}</small></h2>
			@elseif ($days >= 2 and $days <= 6)
				<h2>{{ $now->addDays($days)->format(Config::get('bjga.dates.day.long')) }} <small>{{ $now->addDays($days)->format(Config::get('bjga.dates.dateNoDay')) }}</small></h2>
			@else
				<h2>{{ $now->addDays($days)->format(Config::get('bjga.dates.date')) }}</h2>
			@endif
